...
{{--    </div>--}}
    ...
    <div class="kt-content  kt-grid__item kt-grid__item--fluid" id="kt_content">
                    <input value="{{$id}}" hidden id="project_id" >
...
@section('script')
    ...
    {!! Html::script('assets/app/custom/general/crud/metronic-datatable/advanced/table-report-budget-category.js') !!}
    ...
    <script>
        $(`#kt_form_organisation_unit`).change(function () {
            let dropdown = $(`#kt_form_mission`);
            dropdown.empty();
            let val = $(this).val();
            if (val==null){
                return false;
            ...
            $.ajax({
                url: '{{$nested_url_organisation_unit_multiple}}',
                method: 'POST',
                data: {id: val},
                success: function (data) {
                    ...
                    $('#kt_form_mission').selectpicker('refresh')
                },
                error: function () {
                }
            });
        });
        $(`#kt_form_sector`).change(function () {
            let dropdown = $(`#kt_form_project`);
            dropdown.empty();
            let val = $(this).val();
            if (val==null){
                return false;
            }
            let department_id =$(`#kt_form_department`).val();
            $.ajax({
                url: '{{$nested_sector_project_multiple}}',
                method: 'POST',
                data: {id: val,department_id:department_id},
                success: function (data) {
                    dropdown.append($('<option></option>').attr('value', '').text('All'));
                    $.each(data, function (i, item) {
                        dropdown.append($('<option></option>').attr('value', i).text(item));
                    });
                    $('#kt_form_project').selectpicker('refresh')
                },
                error: function () {
                }
            });
