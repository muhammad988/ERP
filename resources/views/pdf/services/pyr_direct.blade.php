...
<div>
    ...
    <table cellpadding="3" border="1" style="text-align:center">
        ...
        <tr>
            ...
            <td style="background-color:#e1e0dd"></td>
    ...
    <table cellpadding="3" border="1" style="text-align:center">
        ...
        @foreach ($service->service_items as $key => $item)
            <tr>
                ...
                @if ($currency[$item->currency_id] <= $currency[$service->currency_id])
                    <td>{{number_format (($item->quantity * $item->unit_cost) / $item->exchange_rate, 2)}}</td>
                @else
                    <td>{{number_format ($item->quantity * $item->unit_cost * $item->exchange_rate, 2)}}</td>
                @endif
            </tr>
...
{{--            @if ($service->user_exchange_rate != 1)--}}
                <td align="center"  height="30" style="background-color:#e1e0dd">Total (USD) <br/>المجموع</td>
