			 ...
			 if (validator.form()) {
				...
				formEl.ajaxSubmit({
				 ...
				 $.each(validator.errorMap, function (index, value) {
                     ...
                     let name_str_1 = name_str[1];
                     if(name_str[3] != undefined){
						 let name_str_2 = name_str[2].split("]")[0];
						 let name_str_3 = name_str[3].split("]")[0].replace(/_/g, ' ').replace('id', ' ');
						  toastr.error(name_str[0]+' '+name_str_1.split("]")[0]+' '+ name_str_3+' ( '+  Number(Number(name_str_2) + Number(1)) +' ) '+value);
					 }else if(name_str_1 == undefined){
                         toastr.error(name_str[0].replace(/_/g, ' ')+' '+ value);
                     }else{
						 toastr.error(name_str[0].replace(/_/g, ' ')+' '+name_str_1[0].replace(/_/g, ' ')+' '+ value);
					 }
				 });
             }
		});
